...
#include 	"Src/accrete.h"
#include	"Src/enviro.h"
#include	"Src/display.h"
#include	"Src/stargen.h"
#include	"Src/utils.h"
...
#define SET_HLPR( a ) t.a = o.a
SunDat conv( const sun &o ) {
	SunDat t;
	SET_HLPR(luminosity);
	SET_HLPR(mass);
	SET_HLPR(life);
	SET_HLPR(age);
	SET_HLPR(r_ecosphere);
	return t;
}
PlanetDat convP( const planets &o ) {
	PlanetDat t;
	SET_HLPR(planet_no);
	SET_HLPR(a);
	SET_HLPR(e);
	...
	SET_HLPR(moon_a);
	SET_HLPR(moon_e);
	SET_HLPR(core_radius);
	SET_HLPR(radius);
	SET_HLPR(orbit_zone);
	SET_HLPR(density);
	SET_HLPR(orb_period);
	SET_HLPR(day);
	SET_HLPR(resonant_period);
	SET_HLPR(esc_velocity);
	SET_HLPR(surf_accel);
	SET_HLPR(surf_grav);
	SET_HLPR(rms_velocity);
	SET_HLPR(molec_weight);
	SET_HLPR(volatile_gas_inventory);
	SET_HLPR(surf_pressure);
	SET_HLPR(greenhouse_effect);
	SET_HLPR(boil_point);
	SET_HLPR(albedo);
	SET_HLPR(exospheric_temp);
	SET_HLPR(estimated_temp);
	SET_HLPR(estimated_terr_temp);
	SET_HLPR(surf_temp);
	SET_HLPR(greenhs_rise);
	SET_HLPR(high_temp);
	SET_HLPR(low_temp);
	SET_HLPR(max_temp);
	SET_HLPR(min_temp);
	SET_HLPR(hydrosphere);
	SET_HLPR(cloud_cover);
	SET_HLPR(ice_cover);
	SET_HLPR(gases);
	SET_HLPR(type);
	SET_HLPR(minor_moons);
	return t;
}
#undef SET_HLPR
...
int stargen (actions		action,
			 ...
			 CB_System cb_S, CB_Planetoid cb_P
			 )
{
...
//					create_svg_file (sgOut, innermost_planet, path, file_name, ".svg", prognam);
				...
				break;		
				default: {
					cb_S( conv( *innermost_planet->sun ) );
					int pi = 0;
					for (auto planet=innermost_planet; planet != NULL; planet=planet->next_planet) {
						if(planet->sun != innermost_planet->sun) {
							printf("planet->sun != innermost_planet->sun ");
							throw new _exception;
						}
						planet->planet_no = -1;
						cb_P( convP( *planet ) );

						for (auto moon = planet->first_moon; moon != NULL; moon = moon->next_planet ) {
							moon->planet_no = pi;
							cb_P( convP( *moon ) );
						}
						pi++;
...
class StarGen {
public:
	void gen( int seed, CB_System cb_S, CB_Planetoid cb_P ) {
		printf("stargen Test 9 ? \n");
...
#define DLL_INTERFACE(  returnType, className, funcName, paramsT, paramsV, paramsC )	\
	__declspec(dllexport) returnType className##_##funcName paramsT { \
		return SG.funcName paramsC ; \
	}
#include "../../interface_Stargen.h"
}
